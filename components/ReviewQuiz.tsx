...
const categoryLabels: Record<string, string> = {
  diagnosis: "تشخیص",
  investigation: "بررسی‌های تشخیصی",
  treatment: "درمان",
  "follow-up": "پیگیری",
  complication: "عوارض",
  "risk-factor": "عامل خطر",
};
...
export default function ReviewQuiz({ questions }: Props) {
  ...
  return (
    <section className="mt-10 rounded-2xl border border-slate-200 bg-white p-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">مرور سریع</p>
          <h2 className="mt-1 text-xl font-bold text-slate-900">مرور کیس</h2>
        </div>
      ...
      <div className="mt-6">
          ...
          {currentQuestion.options.map((option) => {
            ...
            return (
              <button
                ...
                disabled={answered}
                className={`w-full rounded-xl border p-4 text-right transition ${optionClass}`}
              >
        ...
        {answered && (
          <div className={`mt-5 rounded-xl p-4 ${isCorrect ? "bg-green-50 text-green-900" : "bg-red-50 text-red-900"}`}>
            <p className="font-semibold">{isCorrect ? "صحیح" : "نادرست"}</p>
            {!isCorrect && (
              <p className="mt-1 text-sm">
                پاسخ صحیح: {currentQuestion.options.find((option) => option.id === currentQuestion.correctOptionId)?.text}
              </p>
            )}
        ...
        {answered && isLastQuestion && (
          <div className="mt-5 rounded-xl bg-slate-50 p-4 text-center font-semibold text-slate-700">مرور کیس تمام شد.</div>
        )}
      </div>
    </section>
  );
