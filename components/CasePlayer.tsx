    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-left">
      <div className="mx-auto w-full max-w-4xl rounded-3xl border border-slate-200 bg-white p-6 shadow-xl md:p-10">
        ...
        <section className="mt-5 rounded-2xl border border-blue-200 bg-blue-50/40 p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="text-sm text-slate-500">
              {answered.length} findings
            </span>
          </div>

          {answered.length === 0 ? (
            <p className="mt-3 text-sm text-slate-500">
              Information you collect stays here.
            </p>
          ) : (
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Choose an examination to reveal its findings.
                      </p>
                    </div>

                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500">
                      {physicalOptions.length} available
                    </span>
                  </div>
                ...
                <>
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Choose an investigation to reveal its result.
                      </p>
                    </div>

                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500">
                      {investigationOptions.length} available
                    </span>
                  </div>
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 p-5">
          ...
          >
            Submit Diagnosis —{" "}
            {MAX_GUESSES - guessCount} guesses left
          </button>

          {!questionsComplete && !completed && (
            <p className="mt-3 text-xs text-slate-500">
              You may submit a diagnosis at any point.
              You have 4 total guesses.
            </p>
          )}

          {questionsComplete && !completed && (
            <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-medium text-amber-800">
              You have reached the final finding.
              Submit your diagnosis to finish the case.
            </p>
          )}
